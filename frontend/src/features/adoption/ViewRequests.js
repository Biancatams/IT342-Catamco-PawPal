...
export default function ViewRequests() {
  ...
  const fetchRequests = async () => {
    try {
      const res = await axios.get(
        `https://it342-catamco-pawpal-production.up.railway.app/api/v1/adoption-requests/pet/${petId}`,
        { headers: { Authorization: `Bearer ${token}` } }
      );
  ...
  const handleDeclineConfirm = async () => {
    ...
    try {
      await axios.put(
        `https://it342-catamco-pawpal-production.up.railway.app/api/v1/adoption-requests/${declineModal}/decline`,
  ...
  const handleApprove = async (requestId) => {
    ...
    try {
      await axios.put(
        `https://it342-catamco-pawpal-production.up.railway.app/api/v1/adoption-requests/${requestId}/approve`,
        {},
        { headers: { Authorization: `Bearer ${token}` } }
      );
      await fetchRequests();
                  ...
                  return (
                    <div key={req.id} className={`vr-request-card ${req.status !== "PENDING" ? "vr-request-resolved" : ""}`}>
                      <div className="vr-req-top">
                        {adopterImg ? (
                          <img src={`https://it342-catamco-pawpal-production.up.railway.app${adopterImg}`} alt={req.adopterName} className="vr-req-avatar-img" />
                        ) : (
