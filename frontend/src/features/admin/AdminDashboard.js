...
export default function AdminDashboard() {
  ...
  const fetchAll = async () => {
    try {
      const res = await axios.get("${process.env.REACT_APP_API_URL}/api/v1/pets/admin/all", {
  ...
  const fetchVerifications = async () => {
    ...
    try {
      const res = await axios.get("${process.env.REACT_APP_API_URL}/api/v1/verification/all", {
        headers: { Authorization: `Bearer ${token}` },
      });
  ...
  try {
    const res = await axios.get("${process.env.REACT_APP_API_URL}/api/v1/users/all", {
      headers: { Authorization: `Bearer ${token}` },
    });
...
const fetchReports = async () => {
  ...
  try {
    const res = await axios.get("${process.env.REACT_APP_API_URL}/api/v1/reports/all", {
      headers: { Authorization: `Bearer ${token}` },
    });
...
const handleBanUser = async () => {
  setBanning(true);
  try {
    await axios.put(`${process.env.REACT_APP_API_URL}/api/v1/users/${banModal.userId}/ban`, {}, {
      headers: { Authorization: `Bearer ${token}` },
    });
  ...
  const handleApproveVerif = async (id) => {
    ...
    try {
      await axios.put(`${process.env.REACT_APP_API_URL}/api/v1/verification/${id}/approve`, {}, {
        headers: { Authorization: `Bearer ${token}` },
      });
  ...
  const handleRejectVerif = async () => {
    ...
    try {
      await axios.put(
        `${process.env.REACT_APP_API_URL}/api/v1/verification/${rejectVerifModal.id}/reject`,
  ...
  const handleApprove = async () => {
    ...
    try {
      await axios.put(
        `${process.env.REACT_APP_API_URL}/api/v1/pets/admin/${approveModal.petId}/approve`,
        {},
        { headers: { Authorization: `Bearer ${token}` } }
      );
  ...
  const handleReject = async () => {
    ...
    try {
      await axios.put(
        `${process.env.REACT_APP_API_URL}/api/v1/pets/admin/${rejectModal.petId}/reject`,
        { reason: reasons.join("; ") },
        { headers: { Authorization: `Bearer ${token}` } }
      );
              ...
              >
                ...
                <div style={{ padding: "22px 28px", maxHeight: "78vh", overflowY: "auto" }}>
                  {petDetailModal.imageUrl ? (
                    <img
                      src={`${process.env.REACT_APP_API_URL}${petDetailModal.imageUrl}`}
        ...
        <div className="od-modal-overlay" onClick={() => setViewVerifModal({ open: false, data: null })}>
          <div className="od-modal" style={{ maxWidth: 700, padding: 30, width: "90%" }} onClick={(e) => e.stopPropagation()}>
            <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", marginBottom: 24 }}>
              <div style={{ display: "flex", gap: 20, alignItems: "center" }}>
                ...
                {viewVerifModal.data.user?.profileImageUrl ? (
                  <img src={`${process.env.REACT_APP_API_URL}${viewVerifModal.data.user.profileImageUrl}`} alt="" style={{ width: 80, height: 80, borderRadius: "50%", objectFit: "cover", border: "2px solid var(--border)" }} />
                ) : (
            ...
            <div style={{ background: "#f8f9fa", border: "1px solid var(--border)", borderRadius: 10, padding: 10, textAlign: "center" }}>
              <img 
                src={`${process.env.REACT_APP_API_URL}${viewVerifModal.data.idImageUrl}`} 
                alt="ID Document" 
                ...
                {filtered.map((pet) => (
                  <div key={pet.id} className="admin-listing-card">
                    <div className="admin-listing-img-wrap">
                      {pet.imageUrl ? (
                        <img src={`${process.env.REACT_APP_API_URL}${pet.imageUrl}`} alt={pet.name} className="admin-listing-img" />
                      ) : (
                    ...
                    <div key={v.id} className="admin-listing-card" style={{ flexDirection: "column", padding: 20, gap: 0 }}>
                      ...
                      >
                        {v.user?.profileImageUrl ? (
                          <img src={`${process.env.REACT_APP_API_URL}${v.user.profileImageUrl}`} alt="" style={{ width: 52, height: 52, borderRadius: "50%", objectFit: "cover", border: "2px solid var(--border)", flexShrink: 0 }} />
                        ) : (
                          ...
                          <div style={{ marginBottom: 18 }}>
                            ...
                            <div style={{ display: "flex", flexDirection: "column", gap: 10 }}>
                              {r.reportedUserPets.map(pet => (
                                ...
                                >
                                  {pet.imageUrl ? (
                                    <img src={`${process.env.REACT_APP_API_URL}${pet.imageUrl}`} alt="" style={{ width: 72, height: 72, borderRadius: 8, objectFit: "cover", flexShrink: 0 }} />
                                  ) : (
                      ...
                      {!r.reportedUserBanned && (
                        <div style={{ display: "flex", gap: 10 }}>
                          <button
                            onClick={async () => {
                              try {
                                await axios.put(`${process.env.REACT_APP_API_URL}/api/v1/reports/${r.id}/resolve`, {}, {
                                  headers: { Authorization: `Bearer ${token}` }
                                });
                                setReportDetailModal(null);
                                await fetchReports();
