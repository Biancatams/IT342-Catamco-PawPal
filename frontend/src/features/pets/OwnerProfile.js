...
export default function OwnerProfile() {
  ...
  const fetchMyPets = async () => {
    try {
      const res = await axios.get("${process.env.REACT_APP_API_URL}/api/v1/pets/my", {
        headers: { Authorization: `Bearer ${token}` },
      });
                  ...
                  {pets.map((pet) => (
                    <div key={pet.id} className="prof-pet-item" onClick={() => navigate(`/owner/requests/${pet.id}`, { state: { pet } })}>
                      <div className="prof-pet-thumb-wrap">
                        {pet.imageUrl ? (
                          <img src={`${process.env.REACT_APP_API_URL}${pet.imageUrl}`} alt={pet.name} className="prof-pet-thumb" />
                        ) : (
